<?php use PrivateBin\I18n; $isCpct=s ubstr($template, 9, 8)==='-compact' ; $isDark=s ubstr($template, 9, 5)==='-dark' ; $isPage=s ubstr($template, -5)==='-page' ; ?>
<!DOCTYPE html>
<html>
...
<head>
    ...
    <script defer src="https://use.fontawesome.com/releases/v5.0.1/js/all.js" integrity="sha384-2CD5KZ3lSO1FK9XJ2hsLsEPy5/TBISgKIk2NSEcS03GbEnWEfhzd0x6DBIkqgPN1" crossorigin="anonymous"></script>
    ...
    <?php endif; if ($ZEROBINCOMPATIBILITY): ?>
...
<body role="document" <?php if ($isCpct): ?> class="navbar-spacing"
    <?php endif; ?>>
    ...
    <nav class="navbar navbar-default>
        ...
        <div id="navbar" class="navbar-collapse collapse">
            <ul class="nav navbar-nav">
                ...
                <li><a id="newbutton" type="button" class="hidden"><i class="fas fa-plus"></i> New</a>
                </li>
                <?php if ($EXPIRECLONE): ?>
                <li><a id="clonebutton" type="button" class="hidden"><i class="fas fa-clone"></i> Clone</a>
                </li>
                <?php endif; ?>
                <li><a id="rawtextbutton" type="button" class="hidden"><i class="fas fa-terminal"></i> Raw</a>
                </li>
                <?php if ($QRCODE): ?>
                <li><a id="qrcodelink" data-toggle="modal" data-target="#qrcodemodal" type="button" class="hidden"><i class="fas fa-qrcode"></i> QR</a>
                </li>
                <?php endif; ?>
                <li class="dropdown">
                    <select id="pasteExpiration" name="pasteExpiration" class="hidden">
                        <?php foreach ($EXPIRE as $key=> $value): ?>
                        <option value="<?php echo $key; ?>" <?php if ($key==$ EXPIREDEFAULT): ?> selected="selected"
                            <?php endif; ?>>
                            <?php echo $value; ?>
                        </option>
                        <?php endforeach; ?>
                ...
                <li id="formatter" class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                        <li id="burnafterreadingoption" class="checkbox hidden">
                            <label>
                                <input type="checkbox" id="burnafterreading" name="burnafterreading" <?php if ($BURNAFTERREADINGSELECTED): ?> checked="checked"
                                <?php endif; ?> />
                                <i class="fas fa-fire"></i>
                                <?php echo I18n::_( 'Burn on Open'), PHP_EOL; ?>
                            </label>
                        </li>
                        <?php if ($DISCUSSION): ?>
                        <li id="opendiscussionoption" class="checkbox hidden">
                            <label>
                                <input type="checkbox" id="opendiscussion" name="opendiscussion" <?php if ($OPENDISCUSSION): ?> checked="checked"
                                <?php endif; ?> />
                    ...
                    <select id="pasteFormatter" name="pasteFormatter" class="hidden">
                        <?php foreach ($FORMATTER as $key=> $value): ?>
                        <option value="<?php echo $key; ?>" <?php if ($key==$ FORMATTERDEFAULT): ?> selected="selected"
                            <?php endif; ?>>
                            <?php echo $value; ?>
                        </option>
                        <?php endforeach; ?>
            ...
            <ul class="nav navbar-nav pull-right">
                ...
                <?php endif; ?>
                <li>
                    <li><a id="sendbutton" type="button" class="hidden"><i class="fas fa-arrow-up"></i> Send</a>
                    </li>
                </li>
            </ul>
        </div>
        ...
        <section class="container">
            ...
            <ul id="editorTabs" class="nav nav-tabs hidden">
                <li role="presentation" class="active">
                    <a id="messageedit" href="#">
                        <?php echo I18n::_( 'Editor'); ?>
                    </a>
                </li>
                <li role="presentation">
                    <a id="messagepreview" href="#">
    ...
    <div id="serverdata" class="hidden" aria-hidden="true">
        <div id="cipherdata">
            <?php echo htmlspecialchars($CIPHERDATA, ENT_NOQUOTES); ?>
        </div>
        <?php if ($DISCUSSION): ?>
        <div id="templates">
            ...
            <div id="replytemplate" class="reply hidden">
                ...
                <br />
                <div id="replystatus" role="alert" class="statusmessage hidden alert"><span class="glyphicon" aria-hidden="true"></span> </div>
                <button id="replybutton" class="btn btn-default btn-sm">
